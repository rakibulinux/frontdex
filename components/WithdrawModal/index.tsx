...
import { getAvailableAssets, getLocalAssets } from 'helpers/getAvailableAssets'
import {
    useAuth,
    useContract,
    useCustody,
...
export const WithdrawModal: FC<WithdrawModalProps> = ({
    ...
    const { active, account, signer, chain } = useWallet()
    const [selected, setSelected] = useState(assets[0])
    const [mode, setMode] = useState<string>('withdraw')
    ...
    const preselectedAsset = useAppSelector(state => state.globalSettings.withdrawAsset)
    ...
    useEffect(() => {
        if (!custody || !auth) return

        custody.setAPIKey(auth.getKey())

        if (!custody.apiKey) return
        ;(async () => {
            setLoading(true)
            const assetList = await getAvailableAssets(custody)
            setAvailableAssets(assetList)
        })().catch((error: any) => {
            console.log(error)
            setLoading(false)
    ...
    useEffect(() => {
        const balanceUnitAmount = ethers.utils.parseEther(
            decimalMax(balances.available, balances.decimal),
        )
        if (!amount) return
    ...
    useEffect(() => {
        ...
        ;(async () => {
            setLoading(true)
            const currentBalance = availableAssets.find(
                (a) => a.address === selected.address,
            )

            if (currentBalance) {
                setBalances({
                    locked: currentBalance.locked || 0,
                    available: currentBalance.balances || 0,
                    decimal: 18,
                })
            }
    ...
    }, [selected, tokenContract, availableAssets])

    const handleConfirmWithdraw = useCallback(async () => {
        if (!account || !selected || !amount) return

        setTransactionLoading(true)
        setTransactionError(false)

        try {
            const tx = await custody.withdraw({
                destination: account,
                assets: [
                    {
                        asset: selected.address,
                        amount: ethers.utils.parseEther(decimalMax(amount)),
                    },
                ],
            })
    ...
    const handleConfirmAddAsset = useCallback(
        async (newAsset: any) => {
            ...
            setAvailableAssets(await getAvailableAssets(custody))
    ...
    const handleSelectAsset = useCallback((item: DropdownItem) => {
      const selectedItem = availableAssets.find(a => a.address === item.id.toString()) || assets[0]
            ...
            <React.Fragment>
                <div className="flex justify-between mt-3 text-sm font-medium text-gray-700">
                    ...
                    {loading ? (
                        ...
                        <span>
                            {ethers.utils.commify(balances.available)}{' '}
                ...
                <div className="flex justify-between mt-1 text-sm font-medium text-gray-700">
                    ...
                    {!loading && (
                        <span>
                            {ethers.utils.commify(balances.locked)}{' '}
                            {selected.symbol.toUpperCase()}
                        </span>
                    )}
                </div>
                ...
                <div className="flex justify-left pt-2">
                    <Link href="/history/withdrawal">
                        <a className="text-blue-500 text-sm">
                            See withdraw history
                        </a>
                    </Link>
                </div>
                {transactionError && (
    ...
    const renderWithdraw = useMemo(() => {
        const items: DropdownItem[] = availableAssets.map((a) => {
            return {
                id: a.address,
                name: a.symbol,
        ...
        return (
            <SelectAssetModal
                ...
                onClick={() => valid && handleConfirmWithdraw()}
                ...
                handleRightButtonClick={() =>
                    setAmount(balances.available.toString())
                }
